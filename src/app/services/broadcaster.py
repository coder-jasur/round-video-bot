...
class Broadcaster:
    ...
    async def _update_info_message(
        ...
        try:
            ...
            if include_total:
                text += f"\n\n👥 Jami: {self.total_processed} foydalanuvchi ko'rib chiqildi"
    ...
    async def broadcast(self) -> tuple[int, int, int, int]:
        ...
        info_message_text = (
            ...
            "━━━━━━━━━━━━━━━\n"
            "✉️ yuborib bo'ldi: {sent}\n"
            "❌ Yuborib bo'lmadi: {failed}\n"
            "🚫 Bloklagan: {blocked}\n"
            "🗑️ O'chirilib ketgan akauntlar: {deleted}\n"
            "⛔ Cheklangan akauntlar: {limited}\n"
            "📤 Bazadan oʻchirilgan: {deactivated}\n"
            "🔄 qayta ishlanganlar: {batches}"
        )
