...
@check_sub_channel_router.message(F.text, CommandStart())
@check_sub_channel_router.message(F.text)
async def handle_user_message(message: Message, bot: Bot, conn: Connection, lang: str):
...
async def check_subs_callback(call: CallbackQuery, bot: Bot, conn: Connection, lang: str):
    unsubscribed = await get_unsubscribed_required_channels(
...
@check_sub_channel_router.chat_member(
    ChatMemberUpdatedFilter(IS_NOT_MEMBER >> IS_MEMBER)
)
async def on_user_subscribed_to_channel(
    chat_member: ChatMemberUpdated, bot: Bot, conn: Connection
):
    user_id = chat_member.from_user.id
    channel_id = chat_member.chat.id
    channel_actions = ChannelActions(conn)
    channel_message = await channel_actions.get_channel_message(channel_id)
    ...
    if channel_message:
        if not unsubscribed:
            await bot.send_message(chat_id=user_id, text=str(channel_message))